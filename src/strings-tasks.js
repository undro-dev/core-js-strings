...
function isString(value) {
  ...
  if (value instanceof String) return typeof String(value) === 'string';
  return false;
}
...
function isPalindrome(str) {
  ...
  return str1 === reverseString(str1);
}
...
function encodeToRot13(str) {
  const lowerCseAlphabet = [...'abcdefghijklmnopqrstuvwxyz'];
  const upperCseAlphabet = [...'ABCDEFGHIJKLMNOPQRSTUVWXYZ'];
  let alphabet;
  const str1 = [...str];
  ...
  str1.forEach((el, i) => {
    ...
    if (el === el.toUpperCase()) {
      index = upperCseAlphabet.indexOf(el);
      alphabet = upperCseAlphabet;
    } else {
      index = lowerCseAlphabet.indexOf(el);
      alphabet = lowerCseAlphabet;
    }
    ...
    str1[i] = alphabet[(index + 13) % alphabet.length];
  });
  return str1.join('');
}
